...
## Genre is the form, not the subject

This is the mistake to avoid. Decide what kind of book it is, not what it is about. A poetry
collection about the nineteenth century is `poetry`, not `history-politics`. One person's life
story is `biography-memoir` even when their life was full of politics. A novel set during a war
is `fiction` or `historical`, not `history-politics`. Only use `history-politics` when the book
is itself a work of history or political argument, written to explain events rather than to tell
a story or to be read as verse.
