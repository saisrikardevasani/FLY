"""PDF report generator. Generated from prompt-v1.md. Not edited afterwards."""
...
def build_html(data):
    ...
    <body>
        ...
        <table>
            <tr><th>Title</th><th>Rating</th><th>Price</th></tr>
            {top_rows}
        </table>
...
def create_report(request: ReportRequest = ReportRequest()):
    ...
    generate_pdf(build_html(get_report_data()), output_path)

    conn.execute(
        "INSERT INTO reports (id, path, created_at) VALUES (?, ?, ?)",
        (report_id, str(output_path), datetime.datetime.now().isoformat()),
    )
    conn.commit()
    conn.close()
...
async def get_report(report_id: str):
    ...
    return {**dict(row), "file": f"/reports/{report_id}/file"}
